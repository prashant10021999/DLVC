...
def findEvo():
    ...
    for p in ports:
        ...
        if "5740" in p[2]:
            print('Evo found on port ' + p[0]) # ['COM3','STMicroelectronics Virtual COM Port (COM3)','USB VID:PID=0483:5740 SER=00000000001A LOCATION=1-9']
            return p[0]
    return 'NULL'
...
def openEvo(portname):
    ...
    # Flush in the buffer. tells the serial port to clear the queue so that data doesn't overlap and create erroneous data points
    evo.flushInput()
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if(range==-float('inf')):
                range="Below Min Range"
            else:
                range = str(range)
            print(range + "                         ", end = "\r")
            l.append(range)
            if(len(l)%100==0):
                pickle.dump(l,open('fileevo.pkl','wb'))
